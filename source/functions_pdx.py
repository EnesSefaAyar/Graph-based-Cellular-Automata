...
# Load data
expressions = pd.read_csv("raw_data/CCLE_expression.csv", low_memory=False)
...
def apply_single_mutation(Graph, mutation, randomize=False):
    ...
    if mutation[0] in Graph.nodes:
        if mutation[1]:  # If mutation is deleterious
            if randomize:
...
# Simple progress bar
def progres(current, total, status='', bar_len=30):
