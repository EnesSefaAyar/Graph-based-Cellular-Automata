...
files = os.listdir("tcga_simulations/")
CLs = []
for file in files:
...
for CL in CLs:
    result = fun.find_all_sub_nodes("tcga_simulations/" + CL, tcga=True)
    if result is None:
        continue
    ...
    if len(all_nodes) != 0:
        with open(f"tcga_modules/{CL}.txt", "w") as f1:
        ...
        nx.write_edgelist(sub_net, f"tcga_networks/{CL}.txt")
