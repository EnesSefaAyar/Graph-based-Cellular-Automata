...
def main(cell_line, r):
            ...
            for edge in list_of_graphs[-2].in_edges(node):
                if list_of_graphs[-2].nodes[edge[0]]["activity"] > exp_thr:
                    if list_of_graphs[-2].edges[edge][
                        "is_directed"] != 0:  # If stimulation, estimate is over one std. of mean
    ...
    # Output
    with open(f'random_seeds/{cell_line}_seed_random{r}_states.json', 'w') as fout:
        json.dump(states, fout)
    with open(f'random_seeds/{cell_line}_seed_random{r}_isolated_nodes.json', 'w') as fout:
        json.dump(isolated_nodes, fout)
    with open(f'random_seeds/{cell_line}_seed_random{r}_mutated_nodes.json', 'w') as fout:
        json.dump(mutated_nodes, fout)


cell_line = "ACH-000001"
res = Parallel(n_jobs=-1)(delayed(main)(cell_line, i) for i in range(100))
