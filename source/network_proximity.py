...
mapper = pd.read_csv("raw_data/gene_vs_access_num.txt", delimiter="\t", low_memory=False)
hgncs = list(mapper["Gene name"])
ids = list(mapper["NCBI_ID"])


class DrugTarget(object):
    def __init__(self, dt="raw_data/DrugTargetNetwork.txt"):
        self.d2t = {}
        with open(dt) as fi:
            for line in fi:
                d, t = line.strip("\n").split("\t")
...
class Interactome(object):
    def __init__(self, pathG="raw_data/HumanInteractome.tsv", pathSD="raw_data/HumanInteractome.npy", binSize=300):
...
def proximity(genes1, genes2, repeat, randoms):
    repeat = int(repeat)
    random.seed(int(randoms))

    if genes1 == "DRUG":
        dt = DrugTarget()
        return dt.Screen(genes2, repeat=repeat)
        genes2 = [str(int(ids[hgncs.index(gene)])) for gene in genes2 if gene in hgncs]
    else:
        genes1 = [str(int(ids[hgncs.index(gene)])) for gene in genes1 if gene in hgncs]
        genes2 = [str(int(ids[hgncs.index(gene)])) for gene in genes2 if gene in hgncs]
