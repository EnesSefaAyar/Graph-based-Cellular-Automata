...
def main(cell_line, r):
    ...
    # Randomize edges
    G = f.directed_edge_swap(G, nswap=len(G.edges)*10, max_tries=len(G.edges)*1000)
    ...
    # Output
    with open(f'{cell_line}_edge_random{r}_states.json', 'w') as fout:
        json.dump(states, fout)
    with open(f'{cell_line}_edge_random{r}_isolated_nodes.json', 'w') as fout:
        json.dump(isolated_nodes, fout)
    with open(f'{cell_line}_edge_random{r}_mutated_nodes.json', 'w') as fout:
